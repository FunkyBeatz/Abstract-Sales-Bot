        ...
        logging.basicConfig(filename='./data/logs/bot.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        ...
        class AbstractAPI:
            ...
            def load_tracked_collections(self):
                ...
                try:
                    ...
                    logger.error(f"Failed to load tracked_collections.json: {e}")
                    return {"abstract": {}, "ABS": {}}
            ...
            def connect_to_ws(self):
                ...
                try:
                    ...
                    if self.w3_ws.is_connected():
                        ...
                        logger.warning("WebSocket connection failed; falling back to HTTP")
                        self.w3_ws = None
            ...
            async def listen_for_sales(self):
                ...
                try:
                    ...
                    for blockchain in ["abstract", "ABS"]:
                        for collection, data in self.tracked_collections.get(blockchain, {}).items():
                            contract_address = Web3.to_checksum_address(collection)
                            contract = self.w3_ws.eth.contract(address=contract_address, abi=ERC721_ABI)
                            event_filter = contract.events.Transfer.create_filter(fromBlock="latest")

                            while True:
                                for event in event_filter.get_new_entries():
                                    await self.handle_sale_event(event, contract_address, data)
                        ...
                        for blockchain in ["abstract", "ABS"]:
                            for collection, data in self.tracked_collections.get(blockchain, {}).items():
                                ...
                                events = self.w3_http.eth.get_logs({
                                    ...
                                    'topics': [self.w3_http.keccak(text="Transfer(address,address,uint256)").hex()]
                                })
                                for event in events:
                                    await self.handle_sale_event(event, contract_address, data)
                        break
                    except Exception as e:
                        logger.error(f"HTTP polling attempt {attempt + 1} failed: {e}")
            ...
            async def handle_sale_event(self, event, collection_address, data):
                ...
                try:
                    ...
                    token_id = int(event["topics"][3].hex(), 16) if len(event["topics"]) > 3 else None
                    ...
                    price = Web3.from_wei(tx['value'], 'ether') if tx and tx.get('value', 0) > 0 else None

                    if price or self.is_marketplace_sale(event):  # Add logic for marketplace sales
                        await self.post_sale_to_discord(collection_address, token_id, price, from_addr, to_addr, tx_hash, data['channel_id'])
            ...
            async def post_sale_to_discord(self, collection, token_id, price, buyer, seller, tx_hash, channel_id):
                # Post to Discord (implemented in sales_posting.py)
                from utils.sales_posting import post_sale_to_discord
                await post_sale_to_discord(collection, token_id, price, buyer, seller, tx_hash, channel_id)